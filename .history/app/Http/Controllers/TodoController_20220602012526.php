...
class TodoController extends Controller
{
    ...
    {
        $items = Todo::all();
        $items = Todo::Paginate(10)->orderByDesc('created_at');
...
public function serch(Request $request)
    {
        ...
        if (!empty($toki)){
            if(!empty($toki['from'])){
                $ltoki = Todo::now()->subYear($toki['from'])->format('Y-m-d');
                $query->where('updated_at', '<=', $ltoki);
            }
            if(!empty($toki['to'])){
                $ftoki = Todo::now()->subYear($toki['to'] + 1)->addDay()->format('Y-m-d');
                $query->where('updated_at', '>=', $ftoki);
            }
        }
